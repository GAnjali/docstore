...
class FolderService {
  ...
  static async getAllByParent(userId, parentFolderId) {
    try {
      const result = await database.folder.findAll({
        where: {
          userid: userId,
          parentfolderid: parseInt(parentFolderId) !== 0 ? parentFolderId : null
        }
      });
      return result;
    } catch (error) {
  ...
  static async delete(folderId) {
    try {
      const fileToDelete = await database.folder.findOne({
