...
class FolderController {
  static async create(req, res) {
    ...
    try {
      if (req.body.parentfoldername !== undefined) {
        ...
        if (parentFolder) newFolder.parentfolderid = parentFolder.id;
      }
  ...
  static async getAll(req, res) {
    try {
      ...
      if (allFolders.length > 0) {
        ...
        responseUtil.setSuccess(404, "No folder found");
      }
      return responseUtil.send(res);
    } catch (error) {
  ...
  static async getAllByParent(req, res) {
    try {
      ...
      const allFolders = await FolderService.getAllByParent(
        req.user.id,
        parentFolderId
      );
      if (allFolders.length > 0) {
  ...
  static async delete(req, res) {
    ...
    if (!Number(id)) {
      responseUtil.setError(
        400,
        "Invalid Folder ID, Please input a valid numeric value"
      );
      return responseUtil.send(res);
    }
    try {
